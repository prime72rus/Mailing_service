...
from users.forms import UserRegisterForm, UserProfileUpdateForm, UserManagerForm
...
class BlockedUserView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        if user_to_block.is_superuser and not current_user.is_superuser:
            raise PermissionDenied("Только суперпользователь может блокировать других суперпользователей")
